...
export async function POST(req: Request, res: Response) {
    ...
    try {
        ...
        switch (platform) {
            case "twitter":
                ...
                if (!twitterToken.token || !twitterToken.secret) {
                    return NextResponse.json(
                        { error: "Twitter secrets not found" },
                        { status: 400 }
                    );
                }
                ...
                if (media) {
                    ...
                    mediaIds.push(
                        await twitterClient.v1.uploadMedia(twitterMediaBuffer, {
                            mimeType: "image/png",
                            target: "tweet",
                        })
                    );
                ...
                result = await twitterClient.v2.tweet({
                    ...
                    // If there are media IDs, add them to the tweet (i.e., making it optional)
                    ...(mediaIds.length > 0 && {
                        media: { media_ids: mediaIds as [string] },
                    }),
                });
            ...
            case "linkedin":
                ...
                let linkedInMediaBuffer: Buffer | null = null;
                ...
                //     return NextResponse.json(
                //         { error: "LinkedIn media not found" },
                //         { status: 400 }
                //     );
                // }
                ...
                //     result =
                //         await linkedinPostShare.createPostWithImageForOrganization(
                //             post,
                //             linkedInMediaBuffer,
                ...
                //         );
                // } else {
                //     result = await linkedinPostShare.createPostWithImage(
                //         post,
                //         linkedInMediaBuffer,
                //         imageAlt
                //     );
                ...
                // The following change addresses the issue where media was not being checked (i.e., allowing posts without media)
                if (user.linkedin_company) {
                    result = media // Check media existence here instead
                        ? await linkedinPostShare.createPostWithImageForOrganization(
                              post,
                              linkedInMediaBuffer!,
                              user.linkedin_company,
                        ...
                        : await linkedinPostShare.createPostForOrganization(
                              post,
                              user.linkedin_company
                          );
                } else {
                    result = media // Check media existence here instead
                        ? await linkedinPostShare.createPostWithImage(
                              post,
                              linkedInMediaBuffer!
                          )
                        : await linkedinPostShare.createPost(post);
                }

                break;

            case "bluesky":
                const blueSkyAuth = user.bluesky_token
                    ? JSON.parse(user.bluesky_token)
                    : null;

                if (!blueSkyAuth) {
                    return NextResponse.json(
                        { error: "Bluesky auth not found" },
                        { status: 400 }
                    );
                }
                ...
                result = {
                    ...result,
                    url: `https://bsky.app/profile/${profileName}/post/${postId}`,
                };
        ...
        return NextResponse.json(
            { message: "Error posting", error: error },
            { status: 500 }
        );
    }
}
